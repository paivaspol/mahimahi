...
PacFile::PacFile(const string &path) : path_(path) {}
...
void PacFile::WriteSingleSecureProxy(Address address) {
  stringstream ss;
  ss << "function FindProxyForURL(url, host) {" << endl;
...
void PacFile::WriteProxies(
  ...
  for (auto it = hostnames_to_addresses.begin();
       it != hostnames_to_addresses.end(); ++it) {
...
void PacFile::WriteProxies(vector<pair<string, Address>> hostnames_to_addresses,
  ...
  ss << "\treturn \"DIRECT " << https_default_hostname << ":"
     << to_string(https_default_address.port()) << "\";" << endl;
  ss << "if (shExpMatch(url, \"http:*\")) ";
  ss << "\treturn \"HTTPS " << http_default_hostname << ":"
     << to_string(http_default_address.port()) << "\";" << endl;
  ss << "}";
  ofstream output_file;
  output_file.open(path_);
  output_file << ss.str();
...
void PacFile::WriteProxies(
  ...
  for (uint8_t i = 0; i < hostnames_to_addresses.size(); i++) {
    ...
    // for (auto it = hostnames_to_addresses.begin(); it !=
    // hostnames_to_addresses.end(); ++it) {
    auto hostname_to_address_pair = hostnames_to_addresses[i];
    auto hostname = hostname_to_address_pair.first;
    auto address = hostname_to_address_pair.second;
    auto hostname_to_reverse_server_name_pair =
        hostnames_to_reverse_proxy_name[i];
    ss << "if (shExpMatch(url, \"";
    ss << ((address.port() == 80) ? "http:*" : "https:*");
    ss << hostname_to_address_pair.first << "*";
    ss << "\"))";
    if (address.port() == 80) {
      ss << "\treturn \"HTTPS " << hostname_to_reverse_server_name_pair.second
         << ":" << to_string(address.port()) << "\";" << endl;
    } else {
      ss << "\treturn \"DIRECT\";" << endl;
    }
  }
...
void PacFile::WriteProxies(
  ...
  ss << "\treturn \"HTTPS " << default_hostname << ":"
     << to_string(default_address.port()) << "\";" << endl;
  ss << "}";
  ofstream output_file;
  output_file.open(path_);
  output_file << ss.str();
