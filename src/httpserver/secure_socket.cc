...
public:
  ssl_error(const string &s_attempt, const int error_code = ERR_get_error())
...
SSL_CTX *initialize_new_context(const SSL_MODE type) {
  ...
  SSL_CTX *ret = SSL_CTX_new(type == CLIENT ? TLSv1_2_client_method()
...
SecureSocket::SecureSocket(TCPSocket &&sock, SSL *ssl)
    : TCPSocket(move(sock)), ssl_(ssl), connected_(false) {
  if (not ssl_) {
    throw runtime_error(
...
void SecureSocket::set_tls_hostname(const string &hostname) {
