  ...
  try {
    ...
    if (argc < 8) {
      throw runtime_error("Usage: " + string(argv[0]) +
                          " directory nghttpx_path nghttpx_key "
                          "nghttpx_cert vpn_port path_to_dependency_file "
                          "request_order_filename page");
    }
    ...
    cout << "egress name: " << egress_name << " ingress name: " << ingress_name
         << endl;
    ...
    /* Fork */
    {
      ...
      ChildProcess container_process(
          ...
          [&]() {
            ...
            /* provide seed for random number generator used to create apache
             * pid files */
            srandom(time(NULL));
...

            {
              ...
              /* would be privilege escalation if we let the user read
               * directories or open files as root */

              const vector<string> files = list_directory_contents(directory);

              for (const auto filename : files) {
                FileDescriptor fd(
                    SystemCall("open", open(filename.c_str(), O_RDONLY)));
                ...
                string hostname =
                    HTTPRequest(protobuf.request()).get_header_value("Host");
                hostname_to_ip.emplace_back(hostname, address);
                cout << "Request first line: "
                     << HTTPRequest(protobuf.request()).first_line()
                     << " IP:port: " << address.str() << endl;
            ...
            for (unsigned int server_num = 0; server_num < nameservers.size();
                 server_num++) {
            ...
            DNATWithPostrouting dnat(Address(nameservers[0].ip(), 53), "udp",
                                     53);
            ...
            command.push_back(
                "temp"); // Record path is empty because we are not
                         // actually recording.
            command.push_back(path_to_dependency_file);
            command.push_back(request_order_filename);
            command.push_back(escaped_page);
