...
const char *deepcgi_set_recordingdir(cmd_parms *cmd, void *cfg,
                                     const char *arg) {
  config.recording_dir = arg;
  return NULL;
}

const char *deepcgi_set_loadingpage(cmd_parms *cmd, void *cfg,
                                    const char *arg) {
  config.loading_page = arg;
  return NULL;
}

const char *deepcgi_set_dependencyfile(cmd_parms *cmd, void *cfg,
                                       const char *arg) {
...
static const command_rec deepcgi_directives[] = {
    AP_INIT_TAKE1("workingDir", deepcgi_set_workingdir, NULL, RSRC_CONF,
                  "Working directory"),
    AP_INIT_TAKE1("recordingDir", deepcgi_set_recordingdir, NULL, RSRC_CONF,
                  "Recording directory"),
    AP_INIT_TAKE1("loadingPage", deepcgi_set_loadingpage, NULL, RSRC_CONF,
                  "Loading Page"),
    AP_INIT_TAKE1("dependencyFile", deepcgi_set_dependencyfile, NULL, RSRC_CONF,
                  "Dependency File"),
    {NULL}};
...
module AP_MODULE_DECLARE_DATA deepcgi_module = {
    ...
    deepcgi_directives,      deepcgi_hooks};
...
int deepcgi_handler(request_rec *inpRequest) {
  ...
  /* see bug report at
   * http://modpython.org/pipermail/mod_python/2006-February/020197.html */
  /* taken from fix at
   * https://issues.apache.org/jira/secure/attachment/12321011/requestobject.c.patch
   */
  ...
  do {
    ...
    while (num_bytes_left > 0) {
      ...
      int num_bytes_written =
          ap_rwrite(line + offset, num_bytes_left, inpRequest);
