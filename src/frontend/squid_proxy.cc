...
SquidProxy::SquidProxy(const Address &addr, bool run_after_construction)
    : config_file_("/tmp/squid_config"), moved_away_(false) {
  cout << "[SquidProxy] config: " << config_file_.name()
       << " Address: " << addr.str() << endl;
  ...
  config_file_.write(
      "http_port " + listening_port + " ssl-bump cert=" + path_prefix +
      "/certs/apple-pi.eecs.umich.edu.pem.crt key=" + path_prefix +
      "/certs/apple-pi.eecs.umich.edu.pk generate-host-certificates=on "
  ...
  config_file_.write("pid_filename " + path_prefix + "/var/run/squid_pid." +
...
SquidProxy::~SquidProxy() {
  cout << "[SquidProxy] Stopping SQUID with config " << config_file_.name()
       << endl;
  if (moved_away_) {
    return;
  }
...
void SquidProxy::run_squid() { run({SQUID, "-f", config_file_.name()}); }
