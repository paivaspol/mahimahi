...
int main(int argc, char *argv[]) {
  try {
...

    {
      ...
      /* would be privilege escalation if we let the user read directories or
       * open files as root */
      ...
      for (const auto filename : files) {
        ...
        hostname_to_ip.emplace_back(
            HTTPRequest(protobuf.request()).get_header_value("Host"), address);
      }
    }
    ...
    for (unsigned int server_num = 0; server_num < nameservers.size();
      ...
      cout << "Nameserver: " << nameservers.at(server_num).ip() << endl;
